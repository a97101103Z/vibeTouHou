/**
 * engine.js — Shared game canvas engine used by Playtest and Gauntlet modes.
 *
 * Responsibilities:
 ...
 * - Emit events: 'hit', 'finish', 'restart', 'videoerror'
 */
...
export class GameEngine extends EventTarget {
   ...
   * @param {object} opts.initialPlayer - {x, y} to seed position
   * @param {function} opts.onHit - callback(hits)
   * @param {function} opts.onFinish - callback({hits, trajectory})
   * @param {function} opts.onRestart - callback()
  ...
  _tick(dt) {
    ...
    if (vt >= DURATION || this.video.ended) {
      this._running = false;
      this._drawFrame();
      this._onFinish({ hits: this.hits, trajectory: this.trajectory });
      return;
    }
