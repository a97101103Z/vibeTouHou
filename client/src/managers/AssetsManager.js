...
export class AssetsManager extends EventTarget {
  ...
  #setupEventListeners() {
    ...
    if (this.#btnRefreshAssets) {
      this.#btnRefreshAssets.addEventListener("click", () =>
        this.loadGallery(),
      );
    }
  ...
  async loadGallery() {
    ...
    try {
      ...
      if (!data.files.length) {
        this.#assetGallery.innerHTML =
          '<div style="color:var(--text-muted);font-size:0.82rem;grid-column:1/-1">No assets uploaded yet.</div>';
        this.#files = [];
      } else {
        this.#assetGallery.innerHTML = "";
      ...
      this.#assetGallery.innerHTML =
        '<div style="color:var(--red);font-size:0.82rem;grid-column:1/-1">Could not load assets.</div>';
    }
  }
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          this.dispatchEvent(
            new CustomEvent("upload", { detail: { file: data.filename } }),
          );
        } else {
